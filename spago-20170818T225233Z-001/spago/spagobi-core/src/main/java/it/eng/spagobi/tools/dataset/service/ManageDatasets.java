...
public class ManageDatasets extends AbstractSpagoBIAction {
	...
	private IDataSet getDataSet(String datasetTypeName, boolean savingDataset) throws Exception {
		...
		if (datasetTypeName.equalsIgnoreCase(DataSetConstants.DS_WS)) {
			...
			String wsEndPoint = getAttributeAsString(DataSetConstants.WS_ENDPOINT);
			String wsRequest = getAttributeAsString(DataSetConstants.WS_REQUEST);
			String wsXmlnode = getAttributeAsString(DataSetConstants.WS_XMLNODE);
			jsonDsConfig.put(DataSetConstants.WS_ADDRESS, wsAddress);
			jsonDsConfig.put(DataSetConstants.WS_OPERATION, wsOperation);
			jsonDsConfig.put(DataSetConstants.WS_ENDPOINT, wsEndPoint);
			jsonDsConfig.put(DataSetConstants.WS_REQUEST, wsRequest);
			jsonDsConfig.put(DataSetConstants.WS_XMLNODE, wsXmlnode);
			((WebServiceDataSet) dataSet).setAddress(wsAddress);
			((WebServiceDataSet) dataSet).setOperation(wsOperation);
			((WebServiceDataSet) dataSet).setEndPoint(wsEndPoint);
			((WebServiceDataSet) dataSet).setRequest(wsRequest);
			((WebServiceDataSet) dataSet).setXmlnode(wsXmlnode);
		}
