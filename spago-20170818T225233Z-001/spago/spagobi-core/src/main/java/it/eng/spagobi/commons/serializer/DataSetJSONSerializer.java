...
public class DataSetJSONSerializer implements Serializer {
	...
	private static final String WS_REQUEST = "wsRequest";
	private static final String WS_XMLNODE = "wsXmlnode";
		...
		try {
			...
			try {
				if (type.equalsIgnoreCase(DataSetConstants.FILE) || type.equalsIgnoreCase(DataSetConstants.CKAN)) {
					...
					String ws_endpoint = jsonConf.getString(DataSetConstants.WS_ENDPOINT);
					if (ws_endpoint != null) {
						result.put(WS_ENDPOINT, ws_endpoint);
					}
					
					
					String ws_request = jsonConf.getString(DataSetConstants.WS_REQUEST);
					if (ws_request != null) {
						result.put(WS_REQUEST, ws_request);
					}
					String ws_xmlnode = jsonConf.getString(DataSetConstants.WS_XMLNODE);
					if (ws_xmlnode != null) {
						result.put(WS_XMLNODE, ws_xmlnode);
					}
