	...
	public static IDataSet toDataSet(SbiDataSet sbiDataSet, IEngUserProfile userProfile) {
		...
		try {
			...
			if (sbiDataSet.getType().equalsIgnoreCase(DataSetConstants.DS_WS)) {
				...
				((WebServiceDataSet) ds).setEndPoint(jsonConf.getString(DataSetConstants.WS_ENDPOINT));
				((WebServiceDataSet) ds).setRequest(jsonConf.getString(DataSetConstants.WS_REQUEST));
				((WebServiceDataSet) ds).setXmlnode(jsonConf.getString(DataSetConstants.WS_XMLNODE));
